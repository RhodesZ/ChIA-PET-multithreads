...
while($line = <PAIR>){
	...
	$uid2_head = join(':', $elements[10], $elements[11], $elements[12]);
	$uid2_tail = join(':', $elements[13], $elements[14], $elements[15]);

	$hash{$uid1}{'head'} = $uid1_head;
	$hash{$uid1}{'tail'} = $uid1_tail;
	$hash{$uid1}{'pet'} = $elements[7];
	$hash{$uid2}{'head'} = $uid2_head;
	$hash{$uid2}{'tail'} = $uid2_tail;
	$hash{$uid2}{'pet'} = $elements[17];
...
for($k=0; $k<=$#cc; $k++){
	...
	foreach $m (@members){
		...
		$cluster_size = $cluster_size + $hash{$m}{'pet'};
	}

	$cluster_size = $cluster_size * 0.5;
